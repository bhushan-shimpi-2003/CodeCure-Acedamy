...
export default function Login() {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="flex flex-col items-center justify-center min-h-screen w-full overflow-hidden scanlines matrix-bg selection:bg-emerald-500/30 selection:text-emerald-200 pt-20"
    >
      <div className="w-full max-w-md p-8 bg-[#0a0a0a] border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.15)] relative font-mono z-10">
        ...
        <h2 className="text-2xl font-bold text-center text-white mb-2 uppercase">
          System <span className="text-emerald-500">Login</span>
        </h2>
        ...
        <div className="mt-8 text-center text-sm text-slate-500">
          No access token? <Link to="/signup" className="text-emerald-500 hover:text-emerald-400 underline decoration-emerald-500/30 underline-offset-4">Request access here</Link>.
        </div>
      </div>
    </motion.div>
  );
