...
import { MessageSquare, Terminal, Search, Filter, Eye } from "lucide-react";
      ...
      <div className="border-b border-emerald-500/30 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white mb-1 uppercase flex items-center gap-2">
            <Terminal className="w-6 h-6 text-emerald-500" /> Doubt_Resolution_Log
          </h1>
          <p className="text-emerald-600 text-sm uppercase">&gt; Monitor student doubts and teacher responses.</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-emerald-900/30">
              {doubts.map((doubt, i) => (
                ...
                >
                  <td className="p-4 text-xs text-emerald-500 font-bold">{doubt.id}</td>
                  <td className="p-4 text-sm text-white font-bold">{doubt.student}</td>
                  <td className="p-4 text-sm text-emerald-200">{doubt.subject}</td>
                  <td className="p-4 text-sm text-emerald-600">{doubt.teacher}</td>
                  <td className="p-4">
